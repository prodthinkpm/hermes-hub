# Hermes Hub PRD v0.8 Post-MVP 路线图版

> 基于 v0.7 继续补充：本版新增 **PR1~PR8 完成后的 Post-MVP 路线图**。PR8 完成后先进入 MVP 联调、内测和真实用户验证，而不是立刻扩功能；确认核心假设成立后，再按 P1/P2/P3 分阶段补齐 Profile 创建/克隆、Gateway 管理、日志、Doctor、模板、MCP/Skills/Cron、版本历史和分发发布能力。
...
| `tsconfig.json` | 不存在 |
| Vite / React / Fastify 源码 | 不存在 |
| `packages/` | 不存在 |
| Git 仓库 | 当前检查结果显示不是 Git 仓库 |

产品判断：

当前仓库基本是 PRD 空壳，不适合基于现有工程改造。MVP 应从零搭建 pnpm workspace，但严格避免引入非必要能力。
...
| 包 | 职责 |
|---|---|
| `packages/cli` | 提供 `hermes-hub` / `npx hermes-hub` 启动入口，解析 `--port`、`--host`、`--home`、`--no-open` |
| `packages/server` | Fastify 本地 API 服务，提供 Runtime、Profile、Config、SOUL API，并托管 Web 静态资源 |
| `packages/web` | React + Vite Web UI，提供 Runtime 状态、Profile 列表、Profile 详情、Config / SOUL 编辑 |
| `packages/core` | Hermes Runtime Adapter、Profile 扫描、文件读写、备份、YAML 校验、密钥脱敏 |
| `packages/shared` | 前后端共享类型与 API DTO |

MVP 不引入 SQLite。  
MVP 编辑器优先使用 `textarea`，Monaco Editor 暂缓。  
MVP 不做实时日志、Gateway 管理、Doctor Center、Skills、MCP、Cron、模板系统。
...
1. 所有错误必须结构化返回。
2. Hermes CLI 不存在不能导致 Web 崩溃。
3. HERMES_HOME 不存在应给出明确提示。
4. YAML 校验失败必须阻止保存。
5. 保存前必须备份。
6. 保存失败不能破坏原文件。
7. API 响应和 UI 摘要默认不明文暴露明显密钥字段。

---

### 4. 2 周实施计划

#### Day 1：确认真实 Hermes Runtime 约束

目标：

- 确认 `which hermes`
- 确认 `hermes --version`
- 确认 `hermes profile list`
- 确认 `hermes config home`
- 确认真实 Profile 目录结构
...
- HERMES_HOME 默认探测顺序
- Profile 扫描规则
- Runtime Adapter 输入依据

#### Day 2：搭建 pnpm monorepo 基础骨架
...
- `pnpm install` 后 workspace 能解析依赖。
- TypeScript 基础脚本可运行。
- 包职责边界清晰。

#### Day 3：打通 CLI → Server → Browser 启动链路

交付物：

- `hermes-hub --port --host --no-open`
- Fastify Server 启动
- 默认绑定 `127.0.0.1:8899`
- 端口占用自动选择备用端口
- 自动打开浏览器

#### Day 4：实现 Runtime detect
...
- UI 可展示 Hermes CLI 路径、版本、HERMES_HOME。
- Hermes CLI 不存在时展示清晰错误。
- 探测失败不导致 Web 崩溃。

#### Day 5：实现 Profile 扫描 API 与 Profile List
...
- 能扫描已有 Profile。
- 展示 name、path、config 状态、SOUL 状态、mtime。
- 无 Profile 时给出下一步提示。

#### Day 6：实现 Profile Detail 基础详情
...
- 点击 Profile 可进入详情。
- 配置解析失败时显示“无法解析”，页面不崩溃。

#### Day 7：实现 config.yaml 查看、编辑、YAML 校验
...
- 合法 YAML 可进入保存流程。
- 非法 YAML 阻止保存，并显示错误信息。

#### Day 8：实现 SOUL.md 查看、编辑、基础校验
...
- 可编辑并保存 `SOUL.md`。
- `SOUL.md` 不存在时允许创建，但必须提示“将新建文件”。

#### Day 9：实现保存前备份、原子写入、密钥脱敏
...
- 写入失败不破坏原文件。
- 保存响应包含备份路径。
- UI 与 API 摘要不明文暴露明显密钥。
- 禁止将脱敏占位符误写回真实配置。

#### Day 10：联调、真实 Profile 验证、README 与验收清单
...
- 用户可在 5 分钟内完成：启动 → 探测 → 扫描 → 进入详情 → 编辑 config 或 SOUL → 自动备份 → 保存成功。
- 至少用 1 个真实 Profile 完成验证。
- 非 MVP 功能没有进入主流程。
...
| PR | 内容 | 说明 |
|---|---|---|
| PR 1 | 初始化 pnpm workspace 与 TypeScript 包边界 | 只包含工程骨架，不实现业务 |
| PR 2 | CLI + Fastify 本地启动闭环 | `hermes-hub --port --host --no-open` |
| PR 3 | Shared Types 与 API 错误模型 | 定义 RuntimeInfo、ProfileSummary、ApiError 等 |
| PR 4 | Runtime Adapter detect | 探测 Hermes CLI、版本、HERMES_HOME |
| PR 5 | Profile 扫描与 Profile List 页面 | 扫描目录、空状态、错误状态、列表状态 |
| PR 6 | Profile Detail 与文件状态读取 | config / SOUL 路径、权限、mtime、摘要 |
| PR 7 | config.yaml 编辑与 YAML 校验 | 读取、编辑、校验、错误提示 |
| PR 8 | SOUL.md 编辑、保存前备份、密钥脱敏 | 补齐安全保存闭环 |
...
2. 不允许一次性实现多个阶段。
3. 不允许加入暂不实现清单里的功能。
4. 不允许引入 SQLite，除非明确证明文件扫描无法完成 MVP。
5. 不允许引入 Monaco，除非 textarea 版本已完成且仍有余量。
6. 每个任务必须包含最小可运行验证命令。
7. 每个任务结束后必须输出修改文件列表、运行命令、验证结果、未完成事项。
8. 对涉及文件写入的任务，必须说明备份与失败回滚策略。
...
| 问题 | 建议决策 |
|---|---|
| `hermes profile list` 是否存在 / 是否支持 JSON | Day 1 必须确认 |
| `hermes config home` 返回语义 | Day 1 必须确认 |
| Profile 是多目录还是单目录 registry | Day 1 必须确认 |
| Raw config 是否显示真实密钥 | MVP 建议 Raw 编辑显示真实文件，但保存前检测脱敏占位符，摘要视图脱敏 |
| 备份目录位置 | MVP 使用 `~/.hermes-hub/backups/<profile>/<timestamp>/` |
...
## PR1~PR8 完成后的 Post-MVP 路线图

### 1. 总体判断

PR1~PR8 做完后，Hermes Hub 只完成了 **MVP 最小安全编辑闭环**，还不是完整产品。
...
Profile List
Profile Detail
config.yaml 查看 / 编辑 / YAML 校验 / 保存前备份
SOUL.md 查看 / 编辑 / 保存前备份
基础密钥脱敏
基础错误提示
```

但还不应立即进入大规模功能开发。下一阶段的优先级应该是：
...
### 2. PR8 后第一阶段：MVP 联调与内测准备

#### 2.1 目标

把 PR1~PR8 交付物从“能跑”打磨到“能给真实用户试用”。

#### 2.2 必做任务

| 任务 | 说明 |
|---|---|
| 全链路真实 Profile 验证 | 使用至少 3 个真实 Hermes Profile 测试扫描、读取、编辑、保存 |
| README 完善 | 补充安装、启动、CLI 参数、HERMES_HOME、安全说明 |
| 手动验收清单 | 新增 `docs/manual-test-checklist.md` |
| Demo 数据 | 准备一个 mock HERMES_HOME，方便无 Hermes 环境演示 |
| 错误文案统一 | Hermes CLI 不存在、HERMES_HOME 不存在、权限不足、YAML 错误等 |
| 安全回归 | 验证备份、原子写入、脱敏占位符阻止保存 |
| UI 小修 | 不做大改，只修布局、空状态、错误状态、保存反馈 |
| 打包验证 | 验证 `pnpm build`、本地 CLI 启动、静态资源托管 |
...
```text
PR9：MVP 联调与 README
PR10：错误文案、空状态、加载状态、保存反馈优化
PR11：mock HERMES_HOME 与手动验收脚本
```

---

### 3. PR8 后第二阶段：Alpha 内测

#### 3.1 目标

验证 Hermes Hub 是否真的解决“多个 Hermes Agent 管理麻烦”的核心问题。
...
| 用户类型 | 验证重点 |
|---|---|
| 只有 1 个 Hermes Profile 的用户 | 是否理解产品价值 |
| 有 2~5 个 Hermes Profile 的用户 | Profile 列表和编辑是否有用 |
| 重度配置用户 | config.yaml / SOUL.md 编辑是否比手工更安全 |
| 不熟悉 CLI 的用户 | npx 启动和错误提示是否清楚 |
| 高安全敏感用户 | 密钥脱敏和备份策略是否可信 |

#### 3.3 需要收集的问题

| 问题 | 判断标准 |
|---|---|
| 是否能顺利启动 | 用户是否能独立执行 `npx hermes-hub` |
| 是否能找到 Profile | 扫描逻辑是否适配真实目录 |
| 是否敢在 UI 里编辑配置 | 备份、安全提示、校验是否足够建立信任 |
| 哪个页面最有价值 | Profile List、Detail、Config、SOUL 哪个最常用 |
| 最大阻碍是什么 | CLI、路径、权限、UI、保存安全还是理解成本 |
| 下一个最想要功能 | 创建、克隆、Gateway、日志、Doctor、模板、MCP 哪个优先 |
...
### 4. P1：补齐 Profile 创建与克隆

#### 4.1 为什么优先

PR1~PR8 只解决“已有 Profile 的查看和修改”。  
但用户最初痛点还有一个：**新增 Agent 麻烦**。

因此 Post-MVP 第一个功能增强应该是：
...
| 功能 | MVP 后版本建议 |
|---|---|
| Create Profile | 三步创建：基础信息、模型/SOUL、确认创建 |
| Clone Profile | 从已有 Profile 克隆 config.yaml / SOUL.md，默认不复制密钥、sessions、logs |
| Import Profile | 手动选择目录并注册到 Hermes Hub |
| 删除 Profile | 暂缓，只做“从列表移除”，不删除文件 |
| 模板 | 只做内置模板，不做模板市场 |

#### 4.3 建议 PR

```text
PR12：Create Profile 基础向导
PR13：Clone Profile 安全克隆
PR14：Import Existing Profile
```
...
当用户能创建、编辑 Profile 后，下一步自然是“运行状态和排错”。  
但 Gateway / Logs 容易膨胀，因此要先做轻量版本。

#### 5.2 功能范围

| 功能 | 说明 |
|---|---|
| Gateway 状态查看 | Running / Stopped / Error / Unknown |
| Gateway 启动 | 调用 Hermes CLI 或已确认的命令 |
| Gateway 停止 | 必须确认进程归属，避免误杀 |
| Gateway 重启 | Stop + Start，有明确日志 |
| Logs 查看 | 先做最近 N 行日志，不做实时流 |
| 错误日志高亮 | 仅做关键词高亮 error/warn/failed |

#### 5.3 建议 PR

```text
PR15：Gateway 状态探测
PR16：Gateway Start / Stop / Restart
PR17：基础 Logs 查看
```
...
告警系统
进程守护
systemd / launchd service 管理
```

---

### 6. P3：Doctor / Health Check

#### 6.1 为什么第三优先

Doctor / Health Check 很有产品价值，但前提是前面已经能稳定扫描、编辑、运行。  
否则会变成“看起来智能，但诊断不准”。
...
| 检查项 | 说明 |
|---|---|
| Hermes CLI | 是否存在、版本是否可用 |
| HERMES_HOME | 是否存在、是否可读写 |
| Profile | 目录是否完整 |
...
```text
PR18：Health Check 规则引擎
PR19：Profile Health Summary
PR20：常见问题修复建议
```
...
| 版本历史 | 保存记录列表 |
| 回滚 | 从备份恢复某个文件 |
| Monaco Editor | 替换 textarea |
| 表单配置 | 对常用字段提供可视化编辑 |
| SOUL 模板 | 内置几个常见 Agent 身份模板 |
...
### 8. P5：MCP / Skills / Cron

#### 8.1 为什么后置

MCP、Skills、Cron 是 Hermes Agent 能力扩展的核心，但复杂度高。  
如果过早做，会拖慢主线。
...
| 模块 | 第一版范围 |
|---|---|
| MCP | 列表、启用状态、command/args/env 查看、连接测试 |
| Skills | 列表、启用/禁用、权限风险标记 |
| Cron | 任务列表、启用/禁用、最近执行结果 |
| 批量应用 | 暂缓 |

#### 8.3 建议 PR

```text
PR27：MCP Server 列表与状态
PR28：MCP 配置编辑与连接测试
PR29：Skills 列表与权限风险
PR30：Cron 任务列表
```
...
把 Hermes Hub 从本地开发工具变成可安装、可试用、可反馈的 Alpha 包。

#### 9.2 功能范围

| 功能 | 说明 |
|---|---|
| npm package | 发布 `hermes-hub` 或 `@hermes/hub` |
| npx 验证 | 真实执行 `npx hermes-hub` |
| 版本命令 | `hermes-hub --version` |
| Doctor 命令 | `hermes-hub doctor`，检查自身环境 |
| Demo 模式 | 无 Hermes 时可用 mock 数据演示 |
| 崩溃日志 | 本地保存 Hermes Hub 自身错误日志 |
| Feedback | 引导用户提交 issue |

#### 9.3 建议 PR

```text
PR31：npm package files 与 bin 验证
PR32：npx 安装体验测试
PR33：hermes-hub --version / doctor
PR34：Demo Mode
PR35：Alpha Release Checklist
```

---

### 10. 暂缓到 Beta / v1.0 的功能
...
| 阶段 | 时间 | 目标 |
|---|---|---|
| PR1~PR8 | 2 周 | MVP 最小闭环 |
| PR9~PR11 | 3~5 天 | MVP 联调、README、mock 数据、内测准备 |
| Alpha 内测 | 1 周 | 3~5 个真实用户验证 |
| P1 | 1 周 | Profile 创建 / 克隆 / 导入 |
| P2 | 1 周 | Gateway 状态与基础日志 |
| P3 | 1 周 | Health Check / Doctor |
| P4 | 1~2 周 | 配置体验增强 |
| P5 | 2 周 | MCP / Skills / Cron |
| P6 | 1 周 | npm/npx Alpha 发布 |
...
PR1~PR8 完成后，不要立刻扩展大功能。  
正确顺序是：

```text
PR9~PR11：联调、文档、mock、内测准备
Alpha：找真实用户验证核心假设
P1：Profile 创建 / 克隆 / 导入
P2：Gateway 状态与基础 Logs
P3：Doctor / Health Check
P4：配置体验增强
P5：MCP / Skills / Cron
P6：npx Alpha 发布
```
...
```text
如果用户不愿意用 Hermes Hub 编辑 config.yaml / SOUL.md，
那 Gateway、MCP、模板、智能排障做得再多也没有意义。
```
